...
### Recently shipped
- 2026-06-11: **Two small follow-ups.** (a) The "Show even when total outstanding is 0" sub-toggle is now disabled/greyed (and forced off) when "Show on Dashboard" is off — new `syncShowWhenZeroState()` in `settings.js`, wired to `#card-show-dashboard-input`'s `onchange` and called on modal open (mirrors `syncTrackerToggleState`). (b) Removed the static "Flight 18 · Hotel 36 · Car 6 /₹100" rate line from the HSBC TWP tracker card — it just showed MTD pts + portal spend now. The card always renders when present (gated only by the per-card "Show in Spend Trackers" toggle, like the other trackers); an earlier build briefly hid it at ₹0 spend, reverted. `CACHE_VERSION` v10→v12.
