...
- **PWA install — live verification pending.** Manifest + icons + service worker verified to load/install in preview, but the installed standalone experience (splash screen, status bar, home-screen icon, offline launch) hasn't been checked on a real iPhone. Also app.js's SW-registration call couldn't run in preview (Firebase CDN blocked in sandbox) — confirm the SW actually registers in a real browser via DevTools/Application.
