...
### Recently shipped
- 2026-06-11: **Two small follow-ups.** (a) The "Show even when total outstanding is 0" sub-toggle is now disabled/greyed (and forced off) when "Show on Dashboard" is off — new `syncShowWhenZeroState()` in `settings.js`, wired to `#card-show-dashboard-input`'s `onchange` and called on modal open (mirrors `syncTrackerToggleState`). (b) **HSBC TWP tracker now mirrors the cap-tracker tiles exactly** (Times Black template): `pts / 18,000`, progress bar, `₹X to max cap` + `% used`, Active/Cap-reached badge. New `HSBC_TWP_CAP = 18000` (calendar-month accel-points cap). `pts` = `hsbcTwpPts` (MTD portal points via `hsbcTwpRate`); the "spend to cap" estimate uses the **flights 6× rate (18 pts/₹100)** per Vishal's call (hotel 12× / car 2× reach the cap faster). Always renders when present, gated only by the per-card "Show in Spend Trackers" toggle (the earlier no-cap/portal-spend-line/hide-at-0 variants were all reverted). `CACHE_VERSION` v10→v13.
