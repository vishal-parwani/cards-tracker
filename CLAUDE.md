...
### Recently shipped
- 2026-06-11: **Two small follow-ups.** (a) The "Show even when total outstanding is 0" sub-toggle is now disabled/greyed (and forced off) when "Show on Dashboard" is off — new `syncShowWhenZeroState()` in `settings.js`, wired to `#card-show-dashboard-input`'s `onchange` and called on modal open (mirrors `syncTrackerToggleState`). (b) The HSBC TWP tracker card no longer renders at ₹0 MTD portal spend (`renderHsbcTwp` returns '' when `hsbcTwpSpend <= 0`) — TWP has no cap, so an empty card carried no info; the cap trackers still show 0/cap progress because that's a goal. `CACHE_VERSION` v10→v11.
